...
|------------|-------------|--------|-------|
| K-class | ~10 worlds | ~2-4 | Still too dense |
| M-class | ~7 worlds | ~4-6 | Still too dense |

### Solution: Half Dice Mechanic

Introduced **Half Dice** — using d3 (1-3) instead of d6 (1-6) for body count generation:

| Star Class | Mechanism | Median Bodies |
|------------|-----------|---------------|
| F | Adv+2 on d6 | ~22 |
| G | Baseline d6 | ~15 |
| **K** | **Half Dice (d3) + Dis+3** | **~2** |
| **M** | **Half Dice (d3) + Dis+1** | **~5** |

### Half Dice Implementation

For **K-class** stars (Half Dice + Dis+3):
...
K and M dwarfs should have significantly fewer planets due to:
- Narrower protoplanetary disks
- Lower mass available for planet formation
- Higher stellar wind pressure clearing debris
- Less metallicity in typical red dwarf systems

The Half Dice mechanic achieves appropriate sparsity while maintaining some variability.

### Recommendation for 2026 Book

Update REF-007 to v1.2. Add "Half Dice" term to glossary: rolling d3 (1-3) instead of d6 (1-6), typically combined with Disadvantage for K/M class stars.
...
| K-class modifier | None | **Half Dice + Dis+3** | Adopt v1.2 |
| M-class modifier | Dis+1 | **Half Dice + Dis+1** | Adopt v1.2 |
...
### QA Tracking
All fixes tracked in `QA.md` with IDs QA-001 through QA-015.
