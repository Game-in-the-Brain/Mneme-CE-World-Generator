...
| **QA-069** | ✅ Fixed | Wealth/Development contradiction — renamed UI label from "Material Wealth" to "Resources" to clarify it measures economic output, not population welfare. Expanded `getWealthDevelopmentContext()` with close-rank coherence notes (gap -1, 0, +1) covering resource-curse scenarios: Average+UnderDeveloped (Haiti), Better-off+UnderDeveloped (Equatorial Guinea), Average+Developing (Bangladesh), Prosperous+Developing (China/Brazil), Affluent+Mature (Qatar), Prosperous+Developed (Spain/Italy). Updated Glossary entry with Ghana, Singapore, Japan examples. |
...
| **QA-073** | ✅ Fixed | Low-population culture, power structure, wealth, development, and source-of-power descriptions + labels. All tables (`CULTURE_TRAIT_DESCRIPTIONS_LOW_POP`, `POWER_STRUCTURE_DESCRIPTIONS_LOW_POP`, `SOURCE_OF_POWER_DESCRIPTIONS_LOW_POP`, `WEALTH_DESCRIPTIONS_LOW_POP`, `DEVELOPMENT_DESCRIPTIONS_LOW_POP`, `POWER_STRUCTURE_LABELS_LOW_POP`, `SOURCE_OF_POWER_LABELS_LOW_POP`) exist and are wired into `WorldTab.tsx`, `InhabitantsTab.tsx`, `InhabitantsDemographicsPanel.tsx`, and `OverviewTab.tsx` with `useLowPop` guard. |
