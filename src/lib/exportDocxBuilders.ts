import { Paragraph, Table, TableRow, AlignmentType, WidthType } from 'docx';
...
import { run, h1, h2, line, blank, tCell, tHeaderCell } from './exportDocxHelpers';

// ── Section builders ──────────────────────────────────────────────────────────

export function buildTitle(s: StarSystem): Paragraph[] {
  const date = new Date(s.createdAt).toLocaleString();
  return [
    blank(),
    new Paragraph({
      children:  [run(`${s.primaryStar.class}${s.primaryStar.grade} Star System`, { bold: true, color: '8B2635', size: 56 })],
      alignment: AlignmentType.CENTER,
    }),
    new Paragraph({
      children:  [run('Mneme CE — World Generator', { color: '757575', size: 22 })],
      alignment: AlignmentType.CENTER,
      spacing:   { after: 80 },
    }),
    new Paragraph({
      children:  [run(`Generated: ${date}`, { color: '757575', size: 18, italics: true })],
      alignment: AlignmentType.CENTER,
      spacing:   { after: 400 },
    }),
  ];
...
export function buildStar(s: StarSystem): Paragraph[] {
...
export function buildZones(s: StarSystem): Paragraph[] {
  const z = s.zones;
  return [
    h1('Habitable Zones'),
...
export function buildMainWorld(s: StarSystem): Paragraph[] {
  const w = s.mainWorld;
  return [
    h1('Main World'),
...
export function buildInhabitants(s: StarSystem, shipsInArea?: ShipsInAreaResult | null): Paragraph[] {
  const inh = s.inhabitants;
  if (inh.populated === false) {
    return [
      h1('Inhabitants'),
      new Paragraph({
        children: [run('Unpopulated World — no permanent inhabitants.', { italics: true, color: '757575' })],
      }),
      blank(),
    ];
  }
  ...
  if (shipsInArea && shipsInArea.ships.length > 0) {
    ...
    for (const { label, ships: locShips } of groups) {
      sections.push(new Paragraph({ children: [run(label, { bold: true, color: '1A237E', size: 22 })], spacing: { before: 120, after: 60 } }));
...
export function buildPlanetarySystem(s: StarSystem, annotations: BodyAnnotations): (Paragraph | Table)[] {
  ...
  allPhysBodies.forEach(({ body, idx, isMainWorld }) => {
    ...
    physSections.push(
      new Paragraph({
        children: [
          run(`${idx}. `, { bold: true, color: '757575', size: 18 }),
          run(displayName, { bold: true, size: 18 }),
          run(`  —  ${physBody.zone ?? body.zone ?? '—'}  ·  ${(physBody as PlanetaryBody).moonOrbitAU != null ? (physBody as PlanetaryBody).moonOrbitAU + ' AU orbit' : physBody.distanceAU + ' AU'}`, { color: '757575', size: 18 }),
        ],
